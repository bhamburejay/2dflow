...
public:
  ...
  Vischydro (const Json::Value &in, const EOS &eosin) : inputs(in), eos(eosin) {
    ...
    DMDACreate2d(PETSC_COMM_WORLD,
            ...
            VischydroNode::NDOF,
            stencil_width, nullptr, nullptr, &domain); 
...
PetscErrorCode LHSIFunction(TS ts, PetscReal t, Vec u, Vec udot, Vec F, void *context){
  ...
  VischydroNode *aug;
  // The below line is changed to DMDAVecGetArrayRead because u is locked.
  ...
  DMDAVecRestoreArrayRead(run->domain, u, &aug); // This line was added as it was missing.
...
PetscErrorCode LHSIJacobian(TS ts, PetscReal t, Vec u, Vec udot, PetscReal shift, Mat J, Mat P, void *context)  {
  ...
  // The below line was changed because here we use run->solution_local, not u.
