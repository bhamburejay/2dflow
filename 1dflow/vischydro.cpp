...
public:
  ...
  Vischydro (const Json::Value &in, const EOS &eosin) : inputs(in), eos(eosin) {
    ...
    DMCreateGlobalVector(domain, &solution); // Stores the hydrodynamic variables across the entire domain
    DMCreateLocalVector(domain, &solution_local); // Used for local computations, including ghost cells for interprocess communication.

    // Construct the grid spacing
    xmin = get_inputs("xmin").asDouble();
    xmax = get_inputs("xmax").asDouble();
    // xmin and xmax define the physical extent of the domain
    ...
    TSSetApplicationContext(stepper, this); // Associates the Vischydro object (this) with the time-stepper for callback functions
    TSSetDM(stepper, domain); 
    TSSetType(stepper, TSARKIMEX); // Solver Type (TSARKIMEX): An implicit-explicit method for stiff equations
...
PetscErrorCode LHSIFunction(TS ts, PetscReal t, Vec u, Vec udot, Vec F, void *context){
  ...
  VischydroNode *aug;
  DMDAVecGetArrayRead(run->domain, u, &aug);
  ...
  DMDAVecRestoreArrayRead(run->domain, u, &aug);
