...
PetscErrorCode RunCode() {
  ...
  PetscOptionsGetString(NULL, NULL, "-icfilename", filename, sizeof(filename), NULL);
  vischydro->load_initial_conditions(filename);
  ...
  double cfl = vischydro->get_input({"time_settings", "cfl_max"}).asDouble();
  double dt = cfl * std::min(vischydro->get_dx(), vischydro->get_dy());
  std::cout << "initial_time: " << t_start << std::endl;
  std::cout << "dt: " << dt << std::endl;
  std::cout << "final_time: " << t_end << std::endl;

  PetscErrorCode ierr;
  PetscCall(TSSetTime(vischydro->stepper, t_start));
  PetscCall(TSSetTimeStep(vischydro->stepper, dt));
  PetscCall(TSSetMaxTime(vischydro->stepper, t_end));
  PetscCall(TSSetExactFinalTime(vischydro->stepper, TS_EXACTFINALTIME_STEPOVER));
  TSSetPostStep(vischydro->stepper, PostStepInversion);
