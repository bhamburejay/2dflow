...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gubser Flow Verification Test", usage="%(prog)s {run|plot} [options]")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Sub-command: run --run_command <command>
    run_parser = subparsers.add_parser("run", help="Run the Gubser flow verification test")

    run_parser.add_argument("--run_command", required=True, type=str, default="./VischydroMain.exe", help="Command to run the VischydroMain executable")

    # Sub-command: plot
    plot_parser = subparsers.add_parser("plot", help="Plot results from Gubser flow verification test")
